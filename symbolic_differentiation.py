class expr:
    ...
    def deriv(self,x):
        ...
        if self.variable():
            ...
            e1 = self.addend()
            e2 = self.augend()
            return self.makesum(self.mp(e1,e2.deriv(x)),self.mp(e2,e1.deriv(x)))
        elif self.div():
            e1 = self.addend()
            e2 = self.augend()
            return self.makediv(self.makeminus(self.mp(e1.deriv(x),e2),self.mp(e1,e2.deriv(x))),self.mp(e2,e2))
        else:
